...
const fdpRoutes = require('./routes/fdpRoutes');
const kdpRoutes = require('./routes/kdpRoutes');
const sdpRoutes = require('./routes/sdpRoutes');
...
// MongoDB Atlas connection string   ArOpckExuG7ElmYi
const dbConnectionURL = 'mongodb+srv://prateekrjt14:ArOpckExuG7ElmYi@adminhub.wpq5p.mongodb.net/?retryWrites=true&w=majority&appName=adminHub'
...
app.use('/kdp', kdpRoutes);
app.use('/sdp',sdpRoutes);
app.use('/publications',pubRoutes);
...
app.get('/index', async (req, res) => {
  try {
      ...
      // Render homepage template with class details
      res.render('index', { professorName: professor.professorName, userName: req.session.userName, classDetails });
  } catch (error) {
      console.error('Error fetching class details:', error);
      res.status(500).send('Error fetching class details');
  }
